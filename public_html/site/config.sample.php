 ...
 *
 */
$config->httpHosts = array('dahlenstudio.com','localhost.dahlenstudio.com','www.dahlenstudio.com');
 ...
 *
 */
$config->httpHost = 'dahlenstudio.com';
 ...
 *
 */
$config->adminEmail = 'padi_05@yahoo.com';
 ...
 *
 */
$config->installed = 1469713116;



/*** 12. SITE *********************************************************************************
 *
 * Site specific user defined settings.
 *
 */
...
/**
 * Image size hashes for Zenphoto image URLs.
 * See getImageArgs() @ zp-core/functions-basic.php:868
 */
$img_args = array_pad([], 15, null);
$img_args[3] = $img_args[4] = 0; // cw & ch
$img_args[7] = $config->jpegQuality; // q
$img_args[12] = false; // admin
foreach ([320, 980, 1960] as $imgSize) {
    $img_args[0] = $imgSize;
    $zpSizeHashes["$imgSize"] = sha1('z8Rk=cu7,*eM%CbI>pQ[ZSD({/WLy' . serialize($img_args));
}
$config->set('zpSizeHashes', $zpSizeHashes);
$config->minifyAssets = true;
$config->isLocal = strpos($_SERVER['HTTP_HOST'], 'localhost') !== false;

$config->appConfig = json_encode([
    'ROOT_URL' => $config->urls->root,
    'ALBUMS_URL' => $config->urls->root . 'zenphoto/albums/',
    'DEBUG' => $config->debug,
    'IS_LOCAL' => $config->isLocal,
    'ASSETS_URL' => $config->urls->assets,
    'MINIFY' => $config->minifyAssets,
    'IMG_SIZE_HASHES' => $config->zpSizeHashes,
    'JPEG_QUALITY' => $config->jpegQuality,
]);

function site_error($message, $moreInfo = null)
{
    global $config;
    if ($config->debug) {
        dump($message);
...
$twig = new Twig_Environment(
    new Twig_Loader_Filesystem($config->paths->root . '/site/templates/twig')
);
